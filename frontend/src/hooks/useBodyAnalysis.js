...
export const useBodyAnalysis = (videoRef, stream) => {
  ...
  const [framing] = useState({ todoVisible: false, faltantes: ["todo"] });
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (mounted) {
        setIsReady(true);
        console.log("Pose CDN listo");
      }
    } catch (e) {
      console.error("Error al crear Pose:", e);
    }
    return () => {
      mounted = false;
      if (poseRef.current) {
  ...
  const getSnapshot = useCallback(() => {
    ...
    return {
      ...
      brazos_cruzados: p
        ? p[N.LW].x > p[N.NOSE].x && p[N.RW].x < p[N.NOSE].x
        : false,
      pecho_abierto: true,
