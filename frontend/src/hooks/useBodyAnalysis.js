...
export const useBodyAnalysis = (videoRef, stream) => {
    ...
    useEffect(() => {
        ...
        const loop = async () => {
            ...
            if (!v || v.readyState < 2) { if (running) requestAnimationFrame(loop); return; }
            ...
            try {
                ...
                if (l && l.length >= 25) {
                    ...
                    const bCruz = l[N.LW].x > l[N.NOSE].x && l[N.RW].x < l[N.NOSE].x;
                    const sw = Math.abs(l[N.LS].x - l[N.RS].x);
                    const hw = Math.abs(l[N.LH].x - l[N.RH].x);
                    const pAbierto = hw < 0.01 || sw / hw > 1.1;
                    const de = Math.abs(l[N.LE].x - l[N.RE].x);
                    const ce = (l[N.LE].x + l[N.RE].x) / 2;
                    const mirando = de >= 0.01 && Math.abs(l[N.NOSE].x - ce) / de < 0.2;

                    let score = 100;
                    if (!hBien) score -= 25;
                    if (encorvado) score -= 40;
                    if (bCruz) score -= 20;
                    if (!pAbierto) score -= 15;
                    score = Math.max(0, score);

                    const w = wRef.current;
                    w.push(mirando ? 1 : 0);
                    if (w.length > 30) w.shift();
                    const cv = Math.round((w.reduce((a, b) => a + b, 0) / w.length) * 100);

                    const upd = { posturaScore: score, contactoVisual: cv, frames: mRef.current.frames + 1 };
                    mRef.current = upd;
                    setMetrics(upd);
                    setFraming({ todoVisible: true, faltantes: [] });
                }
            } catch (_) {}
